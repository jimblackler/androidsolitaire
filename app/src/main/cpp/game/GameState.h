...
public:
  ...
  void newGame() {  // todo ... add from settings
    ...
    for (int tableau = 0; tableau != NUMBER_TABLEAUS; tableau++) {
      tableausFaceDown[tableau] = CardList();
      // TODO ... check copy comstructor ... maybe do push
      for (int position = 0; position <= tableau - 1; position++) {
        tableausFaceDown[tableau].add(deck.pop());
      }
      tableausFaceUp[tableau] = CardList();
  ...
  std::list<int> getStack(int cardNumber) {
      int card = cardNumber;
  ...
  std::map<int, std::set<Action>> getActions() {
    ...
    for (int foundationIdx = 0; foundationIdx != NUMBER_FOUNDATIONS;
      ...
      for (auto it = canPlaceOn.begin(); it != canPlaceOn.end(); ++it) {
        int other = *it;
        Action action;
        action.card = other;
